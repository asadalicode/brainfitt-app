...
import { verifyPaymentIntentAPICallVM } from "../virtualMeetingService/virtualMeetingService";

const VirtualMeetingSuccessPaid = () => {
  const search = useLocation().search;
  const paymentStatus = new URLSearchParams(search).get("redirect_status");
  const navigate = useNavigate();
  useEffect(async () => {
    if (paymentStatus === "succeeded") {
      await verifyPaymentIntentAPICallVM();
    }
  }, [paymentStatus]);

  const handleBackButton = () => {
    navigate('/dashboard/virtual-meeting');
  };
...
export default VirtualMeetingSuccessPaid;
