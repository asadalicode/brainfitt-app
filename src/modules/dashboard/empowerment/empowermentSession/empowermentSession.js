...
import { empowermentSessionArray } from "../empowermentSessionArray";
...
import MoodComparisonReport from "../moodComparisonReport/moodComparisonReport";
...
const sessionArrays = empowermentSessionArray;
let CURRENTTASKID;
const EmpowermentSession = ({
  ...
  const getSessionTasks = async () => {
    ...
    if (_response.isSuccess) {
      if (_response.taskList.length > 0) {
        ...
        if (_notificationSessionTask) {
          let _preReqTask = _taskList.find(
            (task) =>
              task.id === _notificationSessionTask.notificationPreviousReq
          );
          if (_preReqTask) {
  ...
  const getNextSessionTask = async (
    isUseCurrentSessionId = false,
    nextSessionIsMaintenance = isMaintenance
  ) => {
    let _nextSessioId = isUseCurrentSessionId ? sessionId : nextSessionId;
    setIsSessionTaskLoading(true);
    let _response = await getSessionTaskAPiCall(
      _nextSessioId,
      nextSessionIsMaintenance
    );
    setIsSessionTaskLoading(false);
  ...
  const manageNextTask = async (nextTaskId, activeSession) => {
    ...
    if (nextTaskId === 0) {
      if (nextSessionIsMaintenance) {
        setIsShowSuccessPopup(true);
        getNextSessionTask(false, true);
        return;
      }
      setIsShowSuccessPopup(true);
      return;
  ...
  const getPostAssessment = async () => {
    ...
    if (_response.isSuccess) {
      ...
      handleToastMessage(
        "warning",
        "Please wait while PDF is beign proccess on server side"
      );
    }
  ...
  const handleTimer = () => {
    let _activeSession = [...activeSessionData];
    let _currentTask = _activeSession.find((task) => task.id === CURRENTTASKID);
    _currentTask.isLock = false;
    setActiveSessionData([..._activeSession]);
